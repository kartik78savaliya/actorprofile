...
import {
  createProfileDb,
  deleteProfileDb,
  findAllProfilesWithAllDetails,
  findProfileByIdWithAllDetails,
  getAllProfiles,
  getProfileByIdDb,
  updateProfileDb,
...
export const listProfiles = asyncHandler(async (req, res) => {
  ...
  if (profiles.length === 0) {
    return res.status(200).json(new ApiResponse(404, "No profiles found"));
  }
  return res
    .status(200)
...
export const deleteProfile = asyncHandler(async (req, res) => {
  ...
  if (deletedProfile) {
    ...
    return res.status(200).json(new ApiError(500, "Failed to delete profile"));
  }
});
...
export const getProfileById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, "Profile details fetched successfully", profile)
    );
});
...
export const shareProfiles = asyncHandler(async (req, res) => {
  ...
  if (
    !toProfileIds ||
    !Array.isArray(toProfileIds) ||
    toProfileIds.length === 0
  ) {
    return res
      .status(400)
      .json(new ApiError(400, "Recipient Profile IDs are required"));
  }
  const profiles = (
    await Promise.all(profileIds.map(findProfileByIdWithAllDetails))
  ).filter((profile) => profile !== null);
  // console.log(profiles,"profiles");

  const recipients = await Promise.all(
    toProfileIds.map((id) => findProfileByIdWithAllDetails(id))
  );
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, "profile shared successfully"));
});
